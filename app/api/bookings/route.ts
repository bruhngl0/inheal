...
const bookingSchema = z.object({
  ...
  service: z.enum([
    "individual_session",
    "group_session",
    "environmental_session",
    "customisable_session",
  ]),
  booking_date: z
    .string()
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, age, phone, service, booking_date, booking_time } =
      validation.data;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM bookings ORDER BY booking_date DESC, booking_time DESC LIMIT $1 OFFSET $2",
      [limit, offset],
    );
